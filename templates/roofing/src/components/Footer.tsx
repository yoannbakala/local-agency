      ...
      <div className="max-w-7xl mx-auto px-4 py-16 grid md:grid-cols-4 gap-10">
        <div>
          ...
          <div className="flex gap-4 mt-5">
            {site.social.google && (
              <a href={site.social.google} target="_blank" rel="noopener noreferrer" className="w-10 h-10 bg-primary-800 rounded-xl flex items-center justify-center text-primary-400 hover:text-accent-400 hover:bg-primary-700 transition-colors" aria-label="Google">
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm">
            {site.services.map(s => (
              <li key={s.slug}>
                <Link href={`/services#${s.slug}`} className="hover:text-accent-400 transition-colors">
                  {s.title}
                </Link>
              </li>
            ))}
